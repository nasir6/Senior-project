...
public class pps{
	...
	pps(byte pps_set_0[]){
		...
		seq_parameter_set_id = p1.ExpGolombDecode();
		entropy_coding_mode_flag=p1.getBit();
		bottom_field_pic_order_in_frame_present_flag=p1.getBit();
		num_slice_groups_minus1=p1.ExpGolombDecode();
		if (num_slice_groups_minus1>0) {
			slice_group_map_type=p1.ExpGolombDecode();
			if (slice_group_map_type==0) {
				run_length_minus1=new int [num_slice_groups_minus1+1];
				for (int igroup=0;igroup<=num_slice_groups_minus1 ;igroup++ ) {
					run_length_minus1[igroup]=p1.ExpGolombDecode();
				...
				for (int igroup2=0;igroup2<num_slice_groups_minus1 ;igroup2++ ) {
					top_left[igroup2]=p1.ExpGolombDecode();
					bottom_right[igroup2]=p1.ExpGolombDecode();

				}
			}else if(slice_group_map_type==3||slice_group_map_type==4||
				slice_group_map_type==5){
				slice_group_change_direction_flag=p1.getBit();
				slice_group_change_rate_minus1=p1.ExpGolombDecode();

			}else if(slice_group_map_type==6){
				pic_size_in_map_units_minus1=p1.ExpGolombDecode();
				slice_group_id=new int[pic_size_in_map_units_minus1];
				for (int id0=0; id0<pic_size_in_map_units_minus1;id0++ ) {
					slice_group_id[id0]=p1.ExpGolombDecode();
				}
			}
		}
		num_ref_idx_l0_default_active_minus1=p1.ExpGolombDecode();
		num_ref_idx_l1_default_active_minus1=p1.ExpGolombDecode();
		weighted_pred_flag=p1.getBit();
		weighted_bipred_idc=p1.readBits(2);
		pic_init_qp_minus26=p1.ExpGolombDecode();
		pic_init_qs_minus26=p1.ExpGolombDecode();

		chroma_qp_index_offset=p1.ExpGolombDecode();
